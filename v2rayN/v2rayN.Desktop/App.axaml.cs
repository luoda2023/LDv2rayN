...
public partial class App : Application
{
 ...
 {
 Logging.SaveLog("OnFrameworkInitializationCompleted", ex);
 Console.Error.WriteLine($"LDv2rayN failed to start:\n{ex}");
...
#region App Event
 ...
 private void Dispatcher_UnhandledException(object? sender, Avalonia.Threading.DispatcherUnhandledExceptionEventArgs e)
    {
        var ex = e.Exception;
        // 先标记 Handled：即使日志写失败也不能让进程被 Avalonia 杀掉。
        e.Handled = true;
        Console.Error.WriteLine($"LDv2rayN UI thread exception:\n{ex}");
        try { Logging.SaveLog("Dispatcher_UnhandledException", ex); }
        catch (Exception logEx) { Console.Error.WriteLine($"Logging failed: {logEx.Message}"); }
    }

 private void CurrentDomain_FirstChanceException(object? sender, FirstChanceExceptionEventArgs e)
 {
 // 仅严重异常落盘，避免刷屏。
 if (e.Exception is AggregateException or ThreadAbortException)
 {
 return;
 }
 if (e.Exception is OutOfMemoryException or StackOverflowException
  or BadImageFormatException or DllNotFoundException or FileNotFoundException)
 {
 Logging.SaveLog("CurrentDomain_FirstChanceException", e.Exception);
 }
 }
